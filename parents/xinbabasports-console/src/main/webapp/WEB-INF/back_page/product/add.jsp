...
<div class="body-box" style="float:right">
	<form id="jvForm" action="/product/toAdd.do" method="post" enctype="multipart/form-data">
		<table cellspacing="1" cellpadding="2" width="100%" border="0" class="pn-ftable">
			<tbody id="tab_1">
				...
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						<span class="pn-frequired">*</span>
						颜色:</td><td id="colorsId" width="80%" class="pn-fcontent">
					</td>
				</tr>
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
			...
			<tbody id="tab_3" style="display: none">
				<tr>
					<td >
						<!-- 加载编辑器的容器 <textarea rows="20" cols="180" id="productdesc" name="description"></textarea>-->
   						 <script id="productdesc" name="description" type="text/plain">
							    这里写你的初始化内容
    					</script>
					</td>
				</tr>
...
UE.Editor.prototype.getActionUrl = function(action) {
    ...
    if (action == 'uploadimage' || action == 'uploadscrawl' || action == 'uploadimage') {
        return 'http://localhost:8084/upload/uploadUE.do';
...
$(function(){
	$.ajax({
		url:"/consoleBrand/brand/sync.do",
		type:"post",
		dataType:"json",
		success:function(data){
			//alert(data[0].name)
			//$("#bad").empty();
			//$("#bad").append("<option value=''>请选择品牌</option>");
			for(var i=0;i<data.length;i++){
				$("#bad").append("<option value='"+data[i].id+"'>"+data[i].name+"</option>");
			}
		}
	}); 
});
$(function(){
	...
	$.ajax({
		...
		success:function(data){
			for(var i=0;i<data.length;i++){
				$("#colorsId").append("<input type='checkbox' value='"+data[i].id+"' name='colors'/>"+data[i].name);
			}
		}
