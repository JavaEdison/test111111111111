...
//登陆
function login(){
	window.location.href = "http://localhost:8085/login/toLogin?resultURL="+ encodeURIComponent(document.location.href);
}
function searchKeyWord(){
	window.location.href = "/portal/search?keyWord="+$("#keyWord").val();
}
function fqBrandId(id){
	if(price==""){
		window.location.href="/portal/search?keyWord="+keyWord+"&brandId="+id;
	}else{
		window.location.href="/portal/search?keyWord="+keyWord+"&price="+price+"&brandId="+id;
	}
}
function fqPrice(price1){
	if(brandId==""){
		window.location.href="/portal/search?keyWord="+keyWord+"&price="+price1;
	}else{
		window.location.href="/portal/search?keyWord="+keyWord+"&price="+price1+"&brandId="+brandId;
	}
}
</script>
</head>
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			<ul class="uls filter">
				...
				<p class="sel">
					...
					<c:forEach items="${filterQuery }" var="fq">
					<em>${fq.key }：</em>${fq.value }
					</c:forEach>
				...
				<li><b>品牌：</b><p>
					...
					<a href="javascript:void(0);" title="${brand.value }" onclick="fqBrandId('${brand.key}')" >${brand.value }</a>					
					</c:forEach>
				</p></li>
				<li><b>价格：</b><p>
			...
			<div class="mt ofc">
				<ul class="uls i_150x150 x4_150x150b">
					...
					<li>
						<a href="javascript:void(0);" onclick="window.open('http://localhost:8081/html/product/${p.id}.html')" title="瑜伽服" target="_blank" class="pic">
						<img src="${p.imgUrl }" alt="瑜伽服" />
						</a>
						<dl>
							<!-- dt 10个文字+... -->
							<dt><a href="javascript:void(0);" onclick="window.open=('http://localhost:8081/html/product/${p.id}.html')" title="依琦莲2014瑜伽服套装新款" target="_blank">${p.name }</a></dt>
							<!-- dt 25个文字+... -->
							<dd class="h40">${p.description }</dd>
							<dd class="orange">￥${p.price }</dd>
							<dd>北京有货</dd>
